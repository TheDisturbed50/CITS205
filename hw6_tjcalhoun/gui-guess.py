...
class Application(Frame): #A GUI application which which generates random number and gets user input

    def __init__(self, master): #initialize Application object
        Frame.__init__(self, master)
        self.grid()
        self.create_widgets()
        self.number = random.randint(1, 21)

    def create_widgets(self): #Get user inputs
        # create instruction label
        Label(self, text = "I'm thinking of a number between 1 and 20.").grid(row = 0, column = 0, sticky = W)
